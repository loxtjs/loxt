...
test('should log error With string', () => {
	...
	expect(spyError).toHaveBeenCalledWith(`${Loxt.format(name, 'error')}: ${Loxt.format(message, TEST)}`);
});
...
test('should log error With Error', () => {
	...
	expect(spyError).toHaveBeenCalledWith(
		error.stack?.replace(error.name, Loxt.format(name, error.constructor.name)).replace(error.message, Loxt.format(message, error.message))
	);
});

test('should have default error', () => {
	const reporter = new Reporter({ info: '', ready: '', start: '', success: '', warn: '' });
...
test('should convert With function', () => {
	expect(loxt.toString()).toBe(
		`${Colors.magenta('class')} ${Colors.yellow('Loxt')} { \x1b[31mreporter\x1b[0m: \x1b[33m${loxt.reporter.constructor.name}\x1b[0m }`
	);
});

test('should convert With static function', () => {
	expect(Loxt.toString()).toBe(`${Colors.magenta('class')} ${Colors.yellow('Loxt')}(${Colors.red('reporter')}: ${Colors.yellow('Reporter')})`);
});
